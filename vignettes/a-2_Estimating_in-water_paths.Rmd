---
title: "2. Estimation of shortest in-water paths"
...
```
library(RSP)
rsp.results <- runRSP(input = filtered_data, coord.x = "Longitude", coord.y = "Latitude", t.layer = base.t)
```
...
RSP also has a `plotDensities()` function, which allows you to investigate the ellapsed time between consecutive detections:
...
We can use `plotTracks()` to perform a quality check in the `runRSP()` output, which by default returns a condensed plot for all tracked animals and allows you to observe whether any strange movements were estimated (e.g. animals crossing through land masses):

USE `suggestSize()` TO FIND BEST SIZE FOR RASTER PLOT!
...
This function is probably obsulete now that we have plotTracks?
