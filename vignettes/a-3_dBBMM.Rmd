...
```
dbbmm.results <- dynBBMM(input = rsp.results, UTM = 32, base.raster = water.shape)
...
```
dbbmm_all <- dynBBMM(input = rsp.data, UTM = 32, base.raster = water.shape, verbose = FALSE)
...
Now that we verified that most of the acoustic detections occurred according to <2 hour time intervals, when the default `maximum.time` of 24 hours was used in `runRSP()`, **timeframe** arguments longer than this value should be safe for analysis. Let's perform a fine-scale analysis using a daily resolution (i.e. `timeframe = 24`):
...
Now you can calculate the areas used with the `getAreas()` function, both at **group** or **track** level, set using the `type` argument. Group areas will correspond to the total areas, within your study site, used by the animals from a particular group. When track areas are set, these are calculated for each animal track individually. Areas are stored in the `$areas` object, as a list of dataframes for each tracked group.
