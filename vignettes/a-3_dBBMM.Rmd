...
A common error you might get when calculating dBBMMs, is when your receivers are **near to the edges** of your base raster. When calculating the space use areas, the dBBMM contours might hit these edges causing the algorithm to crash and stop:
...
This can be simply fixed by increasing the size of your base raster using `loadShape()` and providing a larger `buffer` size. The base raster will be **expanded in all directions** according to the **buffer provided** (meters for metric shapefiles, and degrees for geographic shapefiles):

```
water.large <- loadShape(shape = "my_study_areas.shp", size = 0.0001, buffer = 0.5)  
```
...
You can suppress the individual warning messages by setting `verbose` to FALSE. You will still receive the final message indicating how many detections were excluded. After calculating UDs, the **land areas** are excluded so that the final results represent only **in-water areas of use**.
...
The metadata on the dBBMMs are saved in the `$valid.tracks` object as a data frame:
...
**Note:** You can use the `plotDensities()` function to help you choose the most suitable `timeframe` for `dynBBMM()`, to make sure you won't be mostly using esclusively interpolated locations in your fine-scale analysis:
...
![Here we can notice that most of the consecutive detections occurred at intervals <1 hour for all groups combined (Total), and that **Luderick** was the group with longest mean detection intervals.](plotDensities1.png){#id .class width=100%} 

<br/>

Now that we verified that most of the acoustic detections occurred according to <2 hour time intervals, when the default `max.time` of 24 hours was used in `runRSP()`, **timeframe** arguments longer than this value should be safe for analysis. Let's perform a fine-scale analysis using a daily resolution (i.e. `timeframe = 24`):
...
Please note that you can run the analysis for only a time interval of interest using the `start.time` and `stop.time` arguments. Setting only a **start.time** will cause all data previous to this time to be excluded, and similarly, only a **stop.time** will exclude all the data posterior to the specified time. As in the example, you can also set both arguments to calculate the dBBMMs in a subset of the monitoring period. 

Metadata on the timeslots used in the analysis is returned in the `$timeslots` object:
...
And a new column will be added to the `$valid.tracks` object, representing the corresponding timeslot (Slot) that the dBBMMs were calculated for:
...
You can again use `plotContours()` to visualize the results of timeslot dBBMM, by setting the **timeslot** of interest:
...
```
![You can customize the title of you plot by setting a `title` argument.](plotContours3.png){#id .class width=100%} 


## 3.3. Calculating areas of space use

Now that you have dBBMMs you can calculate the areas used with the `getAreas()` function, both at **group** or **track** level, set using the `type` argument. Group areas will correspond to the total areas used by all animals from a particular group. When track areas are set these are calculated for each animal track individually. Areas are stored in the `$areas` object, as a list of dataframes for each tracked group.
...
If your analysis is of type timeslot dBBMM, a column will be added representing the corresponding timeslots (Slot). Please note the areas of use are calculated by default for the 50% and 95% contours, but can be customized using the `breaks` argument. 
