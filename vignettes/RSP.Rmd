...
Error in runRSP(input = filtered_data, base.raster = "Limfjord_raster.grd") : 
  The location of receiver(s) 'Test1' is in land. Please see ?plotRaster() for details.
```

In this example, `runRSP()` found that the receiver **Test1** is placed in land. We can verify the quality of the base raster provided using `plotRaster()`:

```
plotRaster(study.data, base.raster = "Limfjord_raster.grd")
```
![We can see exactly where **Test1** is placed in land. To fix this error, you can either change the location in the **spatial** input file, or fix the base raster to include the stations **inside the water**.](plotRaster.png){#id .class width=550}
...
We can use `checkLocations()` to perform a quality check in the `runRSP()` output, which returns a condensed plot for all tracked animals and allows you to observe whether any strange movements were estimated (e.g. animals crossing through land masses):
...
We can use `plotDistance()` to compare the total distances travelled by each animal calculated using only the receiver locations and also including the RSP estimations:

![If you set `Group = TRUE` plots are returned individually for each group monitored.](distanceRSP.png){#id .class width=450} 

The `getDistance()` function returns the distances travelled (in km) during each track:
