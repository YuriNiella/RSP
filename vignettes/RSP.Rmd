...
```
library(RSP)
rsp.results <- runRSP(input = filtered_data, base.raster = "Limfjord_raster.grd")
```

Before the analysis start, `runRSP()` verifies the locations of all receivers within the provided base raster. If any receiver is found to be **in land** you will get the following error:

```
Error in runRSP(input = filtered_data, base.raster = "Limfjord_raster.grd") : 
  The location of receiver(s) 'Test1' is in land. Please see ?checkRSPraster() for details.
```

In this example, `runRSP()` found that the receiver **Test1** is placed in land. We can verify the quality of the base raster provided using `checkRSPraster()`:

```
checkRSPraster(study.data, base.raster = "Limfjord_raster.grd")
```
![We can see exactly where **Test1** is placed in land. To fix this error, you can either change the location in the **spatial** input file, or fix the base raster to include the stations **inside the water**.](checkRSP.png){#id .class width=400}
...
RSP also has a `densityRSP()` function, which allows you to investigate the ellapsed time between consecutive detections:
...
We can use `checkRSPlocation()` to perform a quality check in the `runRSP()` output, which returns a condensed plot for all tracked animals and allows you to observe whether any strange movements were estimated (e.g. animals crossing through land masses):

![If you see any **strange behaviours** in this plot it is likely because your **base raster** is not good enought. Changing it's resolution could help obtaining more accurate results. Plese remember to exclude the previous transition layer from your working directory before running `runRSP()` again, or you can use `rmTransition()` to do so automatically.](checkLocations.png){#id .class width=550} 

We can use `distanceRSP()` to compare the total distances travelled by each animal calculated using only the receiver locations and also including the RSP estimations:

![If you set `Group = TRUE` plots are returned individually for each group monitored.](distanceRSP.png){#id .class width=450} 

The same function also returns the distances travelled (in km) during each track:
