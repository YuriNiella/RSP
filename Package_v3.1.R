...
#' 
SPBDdete <- function(location, tz, format.time, tag.data, detect.range = F) {
...
#'
#' @param data Detection dataset imported using the SPBDdete function.
#' 
...
#'
#' @param df.track Detection data for that individual as imported using SPBDdete.
...
#'
#' @param df.detec Detection data for that individual as imported using SPBDdete.
...
#' @param detec.data Detection dataset as returned by SPBDdete.
#' 
...
# Raster file from river (exported from GIS)
...
df.detec <- SPBDdete("Input/Europe data/Limfjord/Limfjord_VEMCO_ATT.csv",
