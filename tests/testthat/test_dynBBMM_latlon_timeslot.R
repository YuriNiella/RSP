...
test_that("actel inputs are working as expected", {
	...
	input$valid.detections[[1]] <- input$valid.detections[[1]][c(148:160), ] # Select 2 valid tracks
...
test_that("runRSP with latlon system is working for timeslot", {
	rsp.data <<- runRSP(input = input, t.layer = tl, coord.x = "Longitude", coord.y = "Latitude", verbose = TRUE)
